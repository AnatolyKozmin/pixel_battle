# 🚀 Запуск приложения на сервере

## Шаг 1: Создайте .env файл

```bash
cd ~/ct/pixel_battle

cat > .env <<EOF
# База данных (внешняя через pgbouncer)
DATABASE_URL=postgresql+asyncpg://pixel_battle_user:pixel_battle_pass@host.docker.internal:6432/pixel_battle_db

# Redis (в контейнере)
REDIS_URL=redis://redis:6379/0

# Telegram (опционально)
TELEGRAM_BOT_TOKEN=

# App
APP_SECRET_KEY=change-me-to-secure-random-string
ALLOWED_ORIGINS=http://YOUR_SERVER_IP:8080,http://YOUR_SERVER_IP:8001

# Frontend
VITE_API_URL=http://YOUR_SERVER_IP:8001
VITE_WS_URL=ws://YOUR_SERVER_IP:8001
EOF
```

**Важно:** Замените `YOUR_SERVER_IP` на реальный IP вашего сервера.

**Как узнать IP:**
```bash
curl ifconfig.me
# или
hostname -I | awk '{print $1}'
```

## Шаг 2: Примените миграции

```bash
# Соберите backend
docker-compose build backend

# Примените миграции
docker-compose run --rm backend alembic upgrade head
```

## Шаг 3: Запустите приложение

```bash
# Соберите все образы (один раз, может занять время)
docker-compose build

# Запустите
docker-compose up -d

# Проверьте статус
docker-compose ps
```

## Шаг 4: Проверка

```bash
# Health check backend
curl http://localhost:8001/health
# Должен вернуть: {"status":"ok"}

# Проверка логов
docker-compose logs -f backend
```

## Шаг 5: Откройте в браузере

```
http://YOUR_SERVER_IP:8080
```

## 🔧 Если что-то не работает

### Backend не запускается

```bash
# Проверьте логи
docker-compose logs backend

# Проверьте подключение к БД
docker-compose run --rm backend python -c "from app.core.database import engine; import asyncio; asyncio.run(engine.connect())"
```

### Ошибка подключения к БД

Если `host.docker.internal` не работает, используйте IP хоста:

```bash
# Узнайте IP хоста
hostname -I | awk '{print $1}'

# Обновите .env
DATABASE_URL=postgresql+asyncpg://pixel_battle_user:pixel_battle_pass@HOST_IP:6432/pixel_battle_db
```

### Frontend не открывается

```bash
# Проверьте логи
docker-compose logs frontend

# Пересоберите frontend
docker-compose build frontend
docker-compose up -d frontend
```

## 📋 Полная последовательность команд

```bash
cd ~/ct/pixel_battle

# 1. Создайте .env (замените YOUR_SERVER_IP)
cat > .env <<EOF
DATABASE_URL=postgresql+asyncpg://pixel_battle_user:pixel_battle_pass@host.docker.internal:6432/pixel_battle_db
REDIS_URL=redis://redis:6379/0
ALLOWED_ORIGINS=http://YOUR_SERVER_IP:8080,http://YOUR_SERVER_IP:8001
VITE_API_URL=http://YOUR_SERVER_IP:8001
VITE_WS_URL=ws://YOUR_SERVER_IP:8001
EOF

# 2. Примените миграции
docker-compose build backend
docker-compose run --rm backend alembic upgrade head

# 3. Запустите
docker-compose build
docker-compose up -d

# 4. Проверьте
docker-compose ps
curl http://localhost:8001/health
```

## ✅ Готово!

Приложение должно быть доступно по адресу: `http://YOUR_SERVER_IP:8080`
